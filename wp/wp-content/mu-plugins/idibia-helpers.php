...
function idibia_pusher_trigger( $channels, string $event_name, array $payload ): bool {
    ...
    if ( idibia_transaction_is_active() ) {
        idibia_queue_pusher_after_commit( $channels, $event_name, $payload );
        return true;
    }

    return idibia_pusher_trigger_now( $channels, $event_name, $payload );
}

function idibia_pusher_trigger_now( $channels, string $event_name, array $payload ): bool {
    if ( ! idibia_pusher_is_configured() ) {
        return false;
    }
...
function idibia_payment_public_payload( int $trip_id, string $viewer_type = 'customer' ): array {
    ...
    if ( $payment ) {
        ...
        if ( $viewer_type !== 'driver' ) {
            $record['proof_path']  = $payment['proof_path'] ?? null;
            $record['proof_url']   = $proof_url;
            $record['admin_notes'] = $payment['admin_notes'] ?? null;
        }
    }
