...
<div class="modal-overlay" id="settingsHistoryModal" style="display:none" onclick="if(event.target===this)closeSettingsHistoryModal()">
  <div class="modal" style="max-width:700px;max-height:90vh;display:flex;flex-direction:column;width:calc(100vw - 32px)">
    <div class="modal-header" style="flex-shrink:0">
      <h3 style="font-size:15px" id="settingsHistoryTitle">Settings — Change History</h3>
      <button aria-label="Close" onclick="closeSettingsHistoryModal()" style="background:none;border:none;font-size:20px;color:var(--text-muted);line-height:1;cursor:pointer">×</button>
    </div>
    <div class="modal-body" style="flex:1;overflow:auto;padding:0">
      <div id="settingsHistoryBody" style="padding:4px 0">
    ...
    <div class="modal-footer" style="flex-shrink:0;justify-content:space-between;flex-wrap:wrap;gap:8px">
      <div id="settingsHistoryPagination" style="display:flex;gap:6px;align-items:center;font-size:12px;color:var(--text-muted)"></div>
      <button onclick="closeSettingsHistoryModal()" style="padding:9px 18px;border-radius:9px;border:1.5px solid var(--surface-2);background:none;font-size:13px;cursor:pointer">Close</button>
    </div>
  </div>
</div>
